...
REASON_CODES:
  ...
  - BROKEN_SPREAD_STATE
  - HUMAN_REARM_REQUIRED
  - ANTI_MARTINGALE_VIOLATION
  - CONCENTRATION_LIMIT_EXCEEDED
  - PROTECTION_HIERARCHY_VIOLATION
  - SECONDARY_FEED_NOT_CERTIFIED
  - EXECUTION_QUALITY_SUSPENDED
  - STRATEGY_NOT_LIVE_APPROVED
```

---

## 17. Model-Cost Controls

Python is cheaper and more reliable than an LLM for deterministic checks. Expensive reasoning models are forbidden in the hot path (see §0.1).

```yaml
MODEL_COST_CONTROLS:
  expensive_reasoning_models_allowed_for:
    - strategy_review
    - failed_trade_postmortem
    - schema_design
    - weekly_performance_review
  cheap_models_or_deterministic_code_required_for:
    - quote_filtering
    - liquidity_checks
    - delta_threshold_checks
    - event_calendar_checks
    - daily_log_formatting
  llm_forbidden_for:
    - final_risk_approval
    - price_validation
    - margin_calculation
